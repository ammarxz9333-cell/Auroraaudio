...
| Live IEC61937 immersive broker | **HOST-PASS** | `aurora-live-ingest` compiles with `-Wall -Wextra -Werror`; it forwards STM32 IEC61937 bytes directly to Omniphony stdin and packetizes rendered 7.1.4 output back to 256-frame `PCM_S32LE`. GitHub host CI passed. Physical service/JOC realtime validation remains mandatory. |
...
| Omniphony 7.1.4 adapter | **STAGED** | Pinned external build and 7.1.4 layout are staged. Its streaming IEC61937 parser is the single demux owner for the live path. End-to-end S6 realtime validation remains physical work. |
...
| STM32 HDMI/eARC IEC61937 capture HAL | **HW-BLOCKED** | Protocol now freezes canonical S16_LE IEC61937 transport; physical HDMI/eARC receiver integration and normalization to that byte stream still require hardware bring-up. |
