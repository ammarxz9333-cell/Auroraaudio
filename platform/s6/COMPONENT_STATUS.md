...
| Aurora source manager control plane | **HOST-PASS** | `aurora-source-manager` owns source priority (`HDMI/eARC > local music > Bluetooth > network`), REVOKE/QUIESCED handoff, a 250 ms stalled-source watchdog, persistent mute/gain/lip-sync/standby state and one-active-source arbitration. CI proves no new source is granted before the old source quiesces, and proves a non-responsive source is disconnected before arbitration continues. This is control-plane proof only; local/Bluetooth/network audio adapters are not yet implemented. |
...
| Source control CLI | **HOST-PASS** | `aurora-source-ctl` is staged into `/usr/local/bin` and host-tested for active-source status plus mute, gain, lip-sync and standby control. It is a diagnostic/UI bridge; the future LVGL UI should keep a persistent control connection rather than shelling out for each action. |
