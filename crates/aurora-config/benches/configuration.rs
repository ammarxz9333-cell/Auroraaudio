...
fn sixteen_speakers() -> AuroraConfiguration {
    let mut config = fixture(include_bytes!(
        "../../../fixtures/config/irregular-horizontal-v3.json"
    ));
...
fn benchmarks(criterion: &mut Criterion) {
    let minimal = fixture(include_bytes!("../../../fixtures/config/minimal-v3.json"));
    ...
    criterion.bench_function("config/migrate_v0_to_v3", |bencher| {
        bencher.iter(|| migrate_v0_to_v3(black_box(migration)).unwrap())
    });
}
