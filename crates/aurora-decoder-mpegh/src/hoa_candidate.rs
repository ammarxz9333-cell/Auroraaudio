...
pub fn render_pure_mpegh_hoa_candidate(
    ...
    if pair.reference_layout.speakers.is_empty() {
        return Err(MpeghHoaCandidateError::MissingReferenceSpeakerLayout);
    }
    ...
    let mut channels = vec![vec![0.0_f32; frame_count]; pair.reference_layout.speakers.len()];
...
pub enum MpeghHoaCandidateError {
    ...
    #[error("paired MPEG-H evidence has no reference speaker geometry")]
