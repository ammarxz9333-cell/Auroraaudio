...
//!
//! `cpal` selects the platform host; on Windows this uses the available Windows
//! audio stack, including WASAPI where supported by CPAL. No CPAL types are
//! exposed through Aurora public APIs.
...
/// CPAL-backed local audio backend.
#[derive(Debug, Default, Clone)]
pub struct CpalAudioBackend;

impl CpalAudioBackend {
    /// Creates a backend using CPAL's default host.
    pub fn new() -> Self {
        Self
    }
}
...
impl CpalStream {
    fn current_fault(&self) -> AudioStreamFault {
        match self.fault.load(Ordering::Acquire) {
            ...
            value if value == AudioStreamFault::FormatChanged as u32 => {
                AudioStreamFault::FormatChanged
            }
...
impl AudioInputStream for CpalStream {
    ...
    fn device_reported_latency_frames(&self) -> Option<usize> {
        self.negotiated.device_reported_latency_frames
    }
...
fn select_device(
    ...
    if let Some(id) = id {
        ...
        let index = selected
            .id
            .parse::<usize>()
            .map_err(|_| RealTimeAudioError::DeviceNotFound(id.to_owned()))?;
        devices
            .into_iter()
            .nth(index)
            .ok_or_else(|| RealTimeAudioError::DeviceNotFound(id.to_owned()))
    } else {
        match direction {
...
mod tests {
    ...
    fn backend_stream_errors_map_to_numeric_faults_without_formatting() {
        assert_eq!(
            stream_error_fault(cpal::StreamError::DeviceNotAvailable),
            AudioStreamFault::DeviceLost
        );
        assert_eq!(
            stream_error_fault(cpal::StreamError::BackendSpecific {
                err: cpal::BackendSpecificError {
                    description: "format changed".to_owned(),
                },
            }),
    ...
    #[test]
    #[ignore = "requires local audio hardware and AURORA_REALTIME_OUTPUT_DEVICE"]
    fn integration_open_output_and_play_silence_then_tone() {
        let output_device_id = std::env::var("AURORA_REALTIME_OUTPUT_DEVICE")
            .expect("set AURORA_REALTIME_OUTPUT_DEVICE to a listed output device id");
        let backend = CpalAudioBackend::new();
        let callbacks = Arc::new(AtomicU64::new(0));
        let callbacks_for_stream = Arc::clone(&callbacks);
        ...
        let mut stream = backend
            .open_output(
                ...
                Box::new(move |output, channels| {
                    callbacks_for_stream.fetch_add(1, Ordering::Relaxed);
                    for frame in 0..(output.len() / channels) {
                        let sample = if callbacks_for_stream.load(Ordering::Relaxed) < 4 {
                            0.0
                        } else {
                            phase.sin() * 0.05
                        };
