...
#define AURORA_REALTIME_MCU_PCM_PERIOD_BYTES \
    (AURORA_USB_PERIOD_FRAMES * AURORA_USB_CHANNELS_7_1_4 * 4u)
#define AURORA_REALTIME_MCU_RX_CAPACITY (16u * 1024u)
...
struct aurora_transport_io {
    ...
    /* Queues exactly one canonical 40-frame / 12-channel / S32LE playback
     * period into the realtime serial-audio path. Return 0 only if ownership
     * was accepted. */
...
struct aurora_transport {
    ...
    uint8_t rx_storage[AURORA_REALTIME_MCU_RX_CAPACITY];
