...
static uint64_t mock_sink_counter(void *opaque) { return ((struct mock_ctx *)opaque)->sink_counter; }
static uint64_t mock_source_counter(void *opaque) { return ((struct mock_ctx *)opaque)->source_counter; }
static uint32_t mock_queued_frames(void *opaque) { return ((struct mock_ctx *)opaque)->queued_frames; }
static void tx_reset(struct mock_ctx *ctx) { ctx->tx_len = 0u; memset(ctx->tx, 0, sizeof(ctx->tx)); }
...
static uint32_t first_flags_for_kind(const struct mock_ctx *ctx, uint16_t wanted)
{
    ...
    while (off + AURORA_USB_HEADER_LEN <= ctx->tx_len) {
        ...
        if (kind == wanted) return aurora_usb_read_le32(h + 8);
        off += frame_len;
    }
    return 0u;
}
static uint64_t first_pts_for_kind(const struct mock_ctx *ctx, uint16_t wanted)
{
    ...
    while (off + AURORA_USB_HEADER_LEN <= ctx->tx_len) {
        ...
        size_t frame_len = AURORA_USB_HEADER_LEN + (size_t)payload_len;
        assert(off + frame_len <= ctx->tx_len);
        if (kind == wanted) return aurora_usb_read_le64(h + 16);
        off += frame_len;
    }
    return UINT64_MAX;
}
...
int main(void)
{
    ...
    struct aurora_realtime_mcu_hal_io io = {0};
    struct mock_ctx ctx = {0};
    uint8_t layout_hash[32];
    uint8_t scratch[64];
    uint32_t slots[4] = {0xf8720000u,0x4e1f0000u,0x00150000u,0x00000000u};
    const uint8_t ping_frame[AURORA_USB_HEADER_LEN] = {
        0x41,0x55,0x52,0x41,0x01,0x00,AURORA_USB_PING,0x00,
    };
    memset(layout_hash, 0x5a, sizeof(layout_hash));
    io.ctx=&ctx; io.usb_send=mock_usb_send; io.queue_pcm_period=mock_queue_pcm;
    io.set_amplifier_mute=mock_set_mute; io.sink_sample_counter=mock_sink_counter;
    io.source_sample_counter=mock_source_counter; io.queued_playback_frames=mock_queued_frames;

    aurora_realtime_mcu_hal_init(&hal, &io, layout_hash);
    assert(ctx.muted == 1 && hal.usb_session_active == 0u && hal.vbus_faulted == 0u);
    assert(aurora_realtime_mcu_hal_usb_receive(&hal, ping_frame, sizeof(ping_frame)) == -2);
    ...
    tx_reset(&ctx);
    assert(aurora_realtime_mcu_hal_earc_dma_s32_high_words(&hal, slots, 4u, 0u, scratch, sizeof(scratch)) == 0);
    assert((first_flags_for_kind(&ctx, AURORA_USB_ENCODED_IEC61937) & AURORA_USB_FLAG_DISCONTINUITY) != 0u);
    ...
    assert(hal.vbus_faulted == 1u && hal.usb_session_active == 0u && ctx.muted == 1);
    aurora_realtime_mcu_hal_vbus_fault_cleared(&hal);
    assert(hal.vbus_faulted == 0u && hal.usb_session_active == 0u);
    aurora_realtime_mcu_hal_usb_session_begin(&hal);

    ctx.sink_counter=1234u; ctx.source_counter=1200u; ctx.queued_frames=80u;
    tx_reset(&ctx);
