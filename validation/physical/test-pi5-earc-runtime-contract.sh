...
lfe=[b for b in blocks if b.startswith('  - name: "LFE"')]
if len(lfe) != 1 or "spatialize: false" not in lfe[0]:
    raise SystemExit("LFE must be the single non-spatialized output")
if sum("spatialize: false" in b for b in blocks) != 1:
    raise SystemExit("only LFE may be non-spatialized")
print("AURORA-11.1.4-LAYOUT-CONTRACT-PASS channels=16")
PY
