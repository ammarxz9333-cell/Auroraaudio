...
payload = {
    ...
    'stable_labels_sha256': __import__('hashlib').sha256(slab.encode()).hexdigest(),
    'candidate_labels_sha256': __import__('hashlib').sha256(elab.encode()).hexdigest(),
...
if ! RUST_LOG="${RUST_LOG:-info}" "$EVAL_OR" "$IEC_FILE" \
  ...
  >"$EVAL_LOG" 2>&1; then
  echo "evaluation Omniphony render failed" >&2
  cat "$EVAL_LOG" >&2 || true
  exit 1
fi
...
python3 - "$EVAL_CONFIG" "$LABEL_REPORT" "$STABLE_RENDER" "$EVAL_RENDER" "$REPORT" "$STABLE_BRIDGE_SHA" "$EVAL_BRIDGE_SHA" "$STABLE_LAYOUT_SHA" "$EVAL_LAYOUT_SHA" "$START_NS" "$BUILD_END_NS" "$RENDER_START_NS" "$RENDER_END_NS" <<'PY'
