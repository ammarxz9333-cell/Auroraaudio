...
static int render_case(const case_spec_t *spec, case_result_t *result) {
  oar_config_t config =
      create_config(ck_oar_layout_stereo, TEST_FRAMES, TEST_SAMPLE_RATE);
  ...
  if (samples_per_channel != TEST_FRAMES || output_channels != 2) {
    fprintf(stderr,
            "unexpected OAR dimensions for %s: frames=%u channels=%u\n",
  ...
  result->finite = 1;
  result->channel_levels[0] =
      channel_rms(output.data, samples_per_channel, &result->finite);
  result->channel_levels[1] = channel_rms(
      output.data + samples_per_channel, samples_per_channel, &result->finite);
  if (!isfinite(result->channel_levels[0]) ||
      !isfinite(result->channel_levels[1])) {
    result->finite = 0;
  }
  free(output.data);
  rc = 0;
...
int main(int argc, char **argv) {
  ...
  const case_spec_t specs[] = {
      {"left_unity", "left", 45.0f, 0.0f},
      {"center_unity", "center", 0.0f, 0.0f},
      {"right_unity", "right", -45.0f, 0.0f},
  ...
  fprintf(stderr, "AURORA-OAR-DIFFERENTIAL-PROBE-PASS\n");
  return 0;
}
