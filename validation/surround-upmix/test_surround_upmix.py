...
def main():
    script = Path(sys.argv[1] if len(sys.argv) > 1 else Path(__file__).with_name("aurora-surround-upmix.sh")).resolve()
    ...
    invalid = subprocess.run(["sh", str(script)], input=b"not an audio carrier", capture_output=True, timeout=10)
    ...
    print(f"PASS: real E-AC-3 IEC61937 -> 7.1.4 ({frames} frames); bed preserved; synthetic heights active; center/LFE/mono isolated; output before EOF; invalid input rejected")
    print(f"HOST ONLY: {duration:.3f}s audio processed in {elapsed:.3f}s ({duration / elapsed:.1f}x offline throughput); no JOC or hardware proof")
