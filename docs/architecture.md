...
- `aurora-runtime-materialization`: isolated near-leaf Checkpoint A owner for bounded passive runtime-resource markers and local invariant errors; no plan derivation or resource construction.
...
ADR 0017 authorizes **Runtime Materialization Contracts 1** as the active
software-only control-plane boundary. Its governance change merged through PR
`#36` at `69cc2e28f8664e724e7677c8be699db84b509d45`. Checkpoint A adds an
`aurora-runtime-materialization` crate that depends directly only on
`aurora-runtime-assembly` and describes aggregate future resource
...
device, or claim runtime readiness. Checkpoint A is `IN_PROGRESS`; Checkpoints
B-D remain `NOT_STARTED`, and no later checkpoint is authorized by this work.
See ADR 0017 and
