...
- `aurora-runtime-inspection`: leaf crate containing a bounded, versioned, redacted-by-default projection of prepared runtime/setup facts; Checkpoint C formatting remains unimplemented.
...
host, or hardware dependency. Checkpoints A and B are `COMPLETE`. Checkpoint B
delivered projection from both prepared plan types, canonical source ordering,
bounded validation, structured errors without silent truncation, default
device/channel/speaker identifier redaction, and explicit local unredacted
options. Checkpoint C is the active next scope but has not started; only its
planned deterministic bounded JSON/text formatting, stable ordering/escaping,
and formatting/determinism tests are authorized next. Final evaluation remains
`NOT_STARTED` Checkpoint D work.
