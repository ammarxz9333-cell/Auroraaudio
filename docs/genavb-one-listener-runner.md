...
The manifest schema is `aurora.genavb.one-listener-run.v1`.
...
- `PHYSICAL-PASS` — only possible without fixture mode when the existing final correlator accepts the independently captured host/NXP/ESP evidence.
...
The self-test contains one positive fixture that must stop at `VALIDATOR-PASS` and one negative RX-counter case that must fail. CI compiles and runs this helper on Linux and Windows.

## Truth boundary

A green CI run proves only that the orchestration logic compiles and preserves the existing fail-closed evidence semantics. It does not prove AVDECC/ACMP interoperability, AAF receive, gPTP lock, synchronization, latency, RF behavior or acoustics on physical hardware.
