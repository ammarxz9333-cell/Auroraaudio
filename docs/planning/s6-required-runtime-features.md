...
## Hardware target ownership

The concrete realtime MCU part/package is selected only in:

```text
config/aurora-hardware-target.env
```

This plan uses the stable symbolic role `AURORA_REALTIME_MCU_ROLE`. CI/build/docs must resolve the concrete target and portable source location through the manifest rather than duplicating the selected part number.
...
- acceptance_dependency_on_phase_2: yes for live HDMI/eARC, physical latency, CEC hardware, wireless endpoints, room measurement, thermals, speaker behavior, and vendor-HAL firmware integration

## Goal

Integrate the runtime features required for the Galaxy S6 + Aurora realtime-MCU appliance path without changing codec ownership or claiming physical validation:

1. consume realtime-MCU CLOCK_REPORT telemetry and drive Aurora's existing PI drift controller plus Rubato ASRC;
...
4. reset decoder/render/DSP state on explicit source/discontinuity events, automatically mark physical IEC61937 carrier-rate changes as discontinuities in the realtime-MCU capture core, and mark the first post-transition PCM period discontinuous; automatic same-rate codec/data-type transition detection remains outside this proof until the real capture/HAL or decoder event boundary supplies it;
...
9. provide one hardware-independent realtime-MCU audio application core that owns transport + capture continuity and exposes a narrow boundary for future USB/audio-DMA/HAL callbacks.
...
- no claim that same-rate codec/data-type changes are detected automatically before the real capture/HAL or decoder event source is integrated;
- no claim that the current portable MCU modules are already linked into a target-specific vendor HAL firmware image; the repository currently proves the portable integration core, not physical DMA/USB callbacks;
...
- 40-frame S6↔realtime-MCU PCM quantum;
- realtime MCU remains physical audio-clock owner;
...
- active MCU vendor/family/part/package remain manifest data, not protocol/API names.
...
- the stateful realtime-MCU capture core marks supported physical carrier-rate transitions (48/96/192 kHz domain) DISCONTINUITY without duplicating IEC61937/Dolby payload parsing;
- automatic same-rate codec/data-type transition detection is an open integration item and is not counted as accepted by the current tests;
- the integrated realtime-MCU audio application core routes USB receive/reset, eARC capture, clock reports, playback PCM, and XRUN recovery through one transport/capture owner;
...
- selected hardware target manifest passes capability/source/status validation;
