...
9. provide one hardware-independent STM32 audio application core that owns transport + capture continuity and exposes a narrow boundary for future USB/SAI/HAL callbacks.
...
- no claim that the current STM32 modules are already linked into a vendor Cube/HAL firmware image; the repository currently proves the portable integration core, not physical DMA/USB callbacks;
...
- the stateful STM32 capture core marks supported physical carrier-rate transitions (48/96/192 kHz domain) DISCONTINUITY without duplicating IEC61937/Dolby payload parsing;
...
- the integrated STM32 audio application core routes USB receive/reset, eARC capture, clock reports, playback PCM, and XRUN recovery through one transport/capture owner;
