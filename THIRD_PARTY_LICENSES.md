...
No Dolby, DTS, HDMI, HDCP, Cavern, `truehdd`, CamillaDSP, libiamf, Harletty, Omniphony, OpenJOC, JOCForge, AOMedia OAR, EBU `libadm`, Fraunhofer `mpeghdec`, Ittiam `libmpegh`, FFmpeg, or wireless-speaker dependency is required to build or test Aurora core. Optional validation lanes may require explicitly pinned external tools. The CPAL backend is isolated in `aurora-realtime-audio-cpal` and is not exposed through Aurora core APIs.
